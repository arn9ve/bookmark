...
export async function geocodeRestaurant(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `La richiesta a Google Places API è fallita con stato: ${response.status}`
      );
    }
    ...
    if (data.status === "OK" && data.results.length > 0) {
      ...
      console.log(
        `[Geocoding] Trovato: "${bestMatch.name}" - ${bestMatch.formatted_address}`
      );
      return {
      ...
      console.warn(
        `[Geocoding] Nessun risultato per: "${query}". Status: ${data.status}`
      );
      return {
        error:
          data.status === "ZERO_RESULTS" ? "No results found" : data.status,
      };
    }
  } catch (error) {
    console.error(
      `[Geocoding] Erre durante la geocodifica per "${query}":`,
      error
    );
    return {
      error:
